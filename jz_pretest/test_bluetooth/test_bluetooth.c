...
uchar hci_set_event_filter[] = { 0x01, 0x05, 0x0C, 0x03, 0x02, 0x00, 0x02 };

uchar hci_write_scan_enable[] = { 0x01, 0x1A, 0x0C, 0x01, 0x03 };

uchar hci_enable_device_under_test_mode[] = { 0x01, 0x03, 0x18, 0x00};

//前面6個字節為固定數值, 下标[6]元素为0x00表示Success; 非00則有問題
uchar success_hci_reset_recevied[] = { 0x04, 0x0e, 0x04, 0x01, 0x03, 0x0c, 0x00 };

//前面6個字節為固定數值, 下标[6]元素为0x00表示Success; 非00則有問題
uchar success_hci_set_event_filter_recevied[] = { 0x04, 0x0e, 0x04, 0x01, 0x05, 0x0c, 0x00 };

//前面6個字節為固定數值, 下标[6]元素为0x00表示Success; 非00則有問題
uchar success_hci_write_scan_enable_recevied[] = { 0x04, 0x0e, 0x04, 0x01, 0x1a, 0x0c, 0x00 };

//前面6個字節為固定數值, 下标[6]元素为0x00表示Success; 非00則有問題
uchar success_hci_enable_device_under_test_mode_recevied[] = { 0x04, 0x0e, 0x04, 0x01, 0x03, 0x18, 0x00 };
...
parse_patchram(char *optarg)
{
	...
	if ((hcdfile_fd = open(optarg, O_RDONLY)) == -1) {
		printf("file %s could not be opened, error %d\n", optarg, errno);
		exit(5);
	}
...
proc_reset()
{
	...
	//检查是否成功,成功返回0，错误返回非0
	if (count == 7)
		return memcmp(buffer,success_hci_reset_recevied,sizeof(uchar)*count);
...
proc_read_bd_addr()
{
	...
	count = read_event(uart_fd, buffer);
	//只要判斷buffer[6]為0x00, 表示Success就可以, 後面的數值沒關係; 非00就是有問題
	if (count == 13 )
		return memcmp(&buffer[6],&check,sizeof(uchar));

	return 1;
}

int
proc_set_event_filter()
{
	int count;
	...
	//检查是否成功，成功返回0，错误返回非0
	if (count == 7)
		return memcmp(buffer,success_hci_set_event_filter_recevied,sizeof(uchar)*count);

	return 1;
}

int
proc_write_scan_enable()
{
	int count;
	...
	//检查是否成功，成功返回0，错误返回非0
	if (count == 7)
		return memcmp(buffer,success_hci_write_scan_enable_recevied,sizeof(uchar)*count);

	return 1;
}

int
proc_enable_device_under_test_mode()
{
	int count;
	...
	//检查是否成功，成功返回0，错误返回非0
	if (count == 7)
		return memcmp(buffer,success_hci_enable_device_under_test_mode_recevied,sizeof(uchar)*count);
...
proc_enable_hci()
{
	...
	if (ioctl(uart_fd, TIOCSETD, &i) < 0) {
		printf("Can't set line discipline\n");
		return;
	}

	if (ioctl(uart_fd, HCIUARTSETPROTO, proto) < 0) {
		printf("Can't set hci protocol\n");
		return;
	}
	printf("Done setting line discpline\n");
	return;
}
...
read_default_bdaddr()
{
	...
	if (sz < 0) {
		...
		printf("read(%s) unexpected size %d", path, sz);
		close(fd);
		return;
	}
...
main (int argc, char **argv)
{
	...
	if (proc_read_bd_addr()) {
		printf("dbaddr\n");
		exit(4);
	}
	if (proc_set_event_filter()) {
		printf ("filter\n");
		exit(5);
	}
	if (proc_write_scan_enable()) {
		printf ("scan_enable\n");
		exit(6);
	}
	if (proc_enable_device_under_test_mode()) {
		printf ("test_mode\n");
		exit(7);
	}
/*
