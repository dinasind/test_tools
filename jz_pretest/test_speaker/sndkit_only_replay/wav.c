		     ...
		     unsigned int *bits)
{
	...
	if (fmt_head->fmt_tag != 1) {
		printf
		    ("WAV: sorry, we current do not support uncompress(fmt_tag = %u)\n",
		     fmt_head->fmt_tag);
		return 0;
	}
