...
#define DEFAULT_SND_DEV	"/dev/snd/dsp"
...
static char optstring[] = "+hVWTD:t:c:f:r:d:B:vPp:Rw:o:";
...
static void usage(void)
{
	...
	printf
	    ("-D --device=NAME\tselect OSS snd device(default is /dev/dsp)\n");
	...
	printf
	    ("-d --duration=#\t\trecord duration default is 10s, or analysis size\n");
...
static int config_device(int fd, int format, int channels, int speed)
{
	...
	if (ioctl_val != format) {
		fprintf(stderr,
			"format not supported, changed by driver to 0x%08x\n",
			ioctl_val);
		return -1;
	}
	...
	if (ioctl_val != channels) {
		fprintf(stderr,
			"%d channel is not supported, changed to %d by driver\n",
			channels, ioctl_val);
		return -1;
	}
	...
	if (ioctl_val != speed) {
		fprintf(stderr,
			"speed %d is not supported, changed to %d by driver\n",
			speed, ioctl_val);
		return -1;
	}
...
static int play(struct replay_params *params)
{
	...
	while (forever || (replay_data_len > 0)) {
		...
		if ((file_type == FILE_TYPE_WAV) && (format == FMT24BITS)) {
			len = (len / 3) * 3;
...
static int record(struct record_params *params)
{
	...
	sndkit_dbg
	    ("REC: format_bits = %d, channels = %d, speed = %d, duration = %d\n",
	...
	while (record_len > 0) {
		n_read = read(dev_fd, buff, (buffer_size > record_len)
		...
		if ((file_type == FILE_TYPE_WAV) && (format_bits == 24)) {
		   ...
		   int channels, int speed, int format)
{
	...
	if ((lstat(raw_file, &stats) == 0) && S_ISREG(stats.st_mode)) {
...
static int strip_wav(char *raw_file, char *wav_file)
{
	...
	if (len != MAX_WAV_HEAD_SIZE) {
		...
		if (wav_data_len == 0) {
			fprintf(stderr,
				"the specified file is not a WAV file.\n");
			return -1;
		}
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		case 'd':
			...
			if (duration == 0) {
				printf
				    ("duration is 0, what do you want to do?\n");
				return 127;
			}
			break;
	...
	if (convert_raw_to_wav)
		return makewav(record_file, replay_file, channels, speed,
			       format);
	...
	if (replay_enable && (replay_file[0] == '\0')) {
		sndkit_dbg
		    ("replay file is not specified! stdin will be used!\n");
		//return 127;
	}
	...
	/* adjust the replay volume */
	{
		...
		if ((mixer_fd = open("/dev/snd/mixer", O_RDWR)) < 0) {
			perror("open /dev/snd/mixer failed.");
			exit(1);
		}
		if (replay_enable) {
			if (ioctl(mixer_fd, SOUND_MIXER_WRITE_VOLUME, &vol) ==
			    -1) {
	...
	if (record_enable && (file_type == FILE_TYPE_WAV)) {
		...
		init_wav_header(wav_head, record_len, channels, speed,
				format_bits);
		hi = WAV_HEAD_SIZE;
		while (hi > 0) {
	...
	if (replay_enable) {
		...
		replaying =
		    pthread_create(&replay_pid, NULL, replay_thread,
				   &replay_params);
		if (replaying != 0) {
	...
	if (record_enable) {
		...
		recording =
		    pthread_create(&record_pid, NULL, record_thread,
				   &record_params);
		if (recording != 0) {
